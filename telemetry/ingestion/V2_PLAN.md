...
Walk into office -> drop the **Sensor Logger ZIP** + the **pre-extracted timing-sheet JSON(s)**
into a watched folder on the Mac -> visualizations and analysis appear automatically.
No manual renaming, no code edits per session. Timing-sheet data is ALREADY extracted to JSON
(no OCR/vision); venue geometry (gate, corners, pit routes) is ALREADY in \_venue/_.geojson + _.md.

## Directory layout (kart_v2/)

```
inbox/          <- DROP ZONE: put SensorLogger .zip + timing-sheet *.json here
...
### **_ THE AUDIO SYNC BUG (root cause, finally) _**
...
- Clear **pycache** when switching session config (stale imports gave wrong/old output).
...
1. **ingest.py**: watch inbox/ -> for each .zip: unzip to raw_sessions/<name>/; read metadata.csv;
   keep ORIGINAL filenames; LOAD the pre-extracted timing-sheet \*.json (no OCR/vision).
   Auto-detect multiple sessions within one recording (gap analysis on GPS + lap clustering).
2. **timing_sheet.py**: LOAD the dropped \*.json (already parsed; lap times, kart#, datetime,
   ProSkill, leaderboards). No Vision/OCR. Output OFFICIAL[] + best lap per session.
...
- Timesheet data: **already extracted to JSON** (e.g. `2026-06-25_1440.json`,
  `2026-06-25_1500.json`) in the venue inbox. NO vision/OCR is performed by the pipeline.
  The JSON carries datetime, driver, kart#, per-lap times, best lap, ProSkill, leaderboards.
  The JSON's own `date`+`time` fields are the authoritative session date/time.
...
label FP1/FP2 - the pipeline figures out sessions from the data + timing-sheet JSON.

```
inbox/
  gateway-kartplex/
    World_Wide_Technology_Raceway-2026-06-25_21-02-36.zip  <- the ONE recording (may hold N sessions)
    2026-06-25_1440.json   <- timing-sheet data for session A (pre-extracted)
    2026-06-25_1500.json   <- timing-sheet data for session B (pre-extracted)
```
...
- `*.json` = pre-extracted TIMING SHEET data (datetime, lap times, kart#, etc).
- Venue geometry (gate, corners, pit routes, track map) lives ONCE in
  `output/<venue>/_venue/` as `gateway_kartplex_t1.geojson` + `gateway_kartplex_t1.md`.
  These are read directly; the pipeline does NO image classification or vision.
...
2. Read EACH timing-sheet JSON -> datetime (AUTHORITATIVE, see below) + lap-count + lap-time fingerprint.
...
### AUTHORITATIVE session datetime (from the sheet JSON)

The timing-sheet JSON carries `date` + `time` (e.g. `2026-06-25` `14:40`). USE THIS DATE+TIME
as the authoritative session identity / ordering / output name / weather lookup. The JSON also
carries `visitNumber` and `raceNumber` - CAPTURE these into timesheet.json but they are SOMETIMES
WRONG: never use visit#/race# for splitting, ordering, or naming. There is NO FP1/FP2 label on
A&D sheets -> name sessions by datetime.
  ...
  gateway-kartplex/
    2026-06-25_14-40/          <- {sheet-datetime}; (+_Type only if sheet states one)
      ...
      dataset/                 <- THE CANONICAL PRODUCT (consumers read this, never re-derive)
        ...
        timesheet.json         <- a copy of the pre-extracted sheet JSON (laps, kart#, datetime,
                                  driver, ProSkill, position, leaderboards)
...
ingest(zip + timing JSON) -> raw/ -> derive datetime -> sync(drift fit) -> fuse(GPS spline+ZUPT)
-> detect+validate flying laps vs timesheet -> write dataset/. Pure data. No plots. No vision.
...
1. ingest: scan inbox/<venue>/ flat. INPUTS = exactly one .zip (recording) + one-or-more
   timing-sheet \*.json (pre-extracted). IGNORE everything else gracefully (.DS_Store, stray
   files - do not error). Unzip -> raw/; read metadata.csv (device, schema v, recording epoch,
   sensor list/rates). NO image classification / NO vision - timing data is already in JSON,
   venue geometry is already in \_venue/\*.geojson + \*.md.
2. timesheet: LOAD the pre-extracted \*.json per session (no OCR/vision). It already contains:
   - datetime (`date`+`time`, authoritative session date/time) + venue + session format ("10 min A&D")
   - driver name, KART NUMBER (matters; even "equal" karts vary), position/result
   - per-lap times[], best_lap, #laps, ProSkill rank+points, leaderboards
   - visitNumber/raceNumber (captured, NOT used for split/order/name)
     Copy the JSON verbatim into dataset/timesheet.json.
...
6. laps: fixed gate (lat/lon based, FROM \_venue/\*.geojson sf_gate_line) crossing + auto-align to
   timesheet (slide window, drop out-lap). Mark flyers; isolate from out/in/pit.
7. write dataset/ (+ aligned_100hz.parquet). Done. Hand off to consumers.

### Gate / sectors (make venue-relative, not anchor-relative)

v1 gate was ENU coords tied to one recording's anchor. v2: read gate + corners DIRECTLY from
`output/<venue>/_venue/gateway_kartplex_t1.geojson` (`sf_gate_line` segment as lat/lon endpoints,
`turn_1`..`turn_11` apex seeds). Snap each to the GPS track + validate by data (gate via lap-time
RMSE; corners via curvature peaks). Convert to each session's ENU at fuse time. The 11 named
corners come from the venue .md/.geojson - NOT re-derived by vision. Sectors as fractions
[0,0.33,0.63,1.0] = S1 ESSES, S2 STRAIGHT, S3 HAIRPIN (Gateway Kartplex T1). Re-validate per venue.
...
### **_ APPROXIMATE / ROUGH SKETCH - DO NOT BE LITERAL _**
...
- Venue geometry is provided ONCE as `output/<venue>/_venue/gateway_kartplex_t1.geojson` +
  `gateway_kartplex_t1.md` (corners, gate, pit routes, racing direction). The pipeline reads
  these files directly. NO image classification, NO vision step.
- The Gateway Kartplex T1 reference encodes: 11 numbered corners (T1-T11, CCW), the S/F gate
  segment, pit-out / pit-in route sketches, and the apex-to-apex layout (~539 m lower bound).
- Any satellite screenshot / hand-drawn map remains in `_venue/` as human reference only; the
  pipeline does not process images.

### sectors.json carries REAL corner identity (do this in v2)

Build sectors.json by reading the 11 corner apex seeds (`turn_1`..`turn_11`) from the venue
geojson, snapping them to the validated best-lap GPS, and assigning dist-% along the lap. The
corner identity comes from the venue .geojson/.md - NEVER from per-session vision. Goal: analysis
speaks driver language ("lost 0.2s in Turn 4") not "sector 2".
...
- Corner dist-% are assigned by snapping the venue geojson apex seeds (`turn_1`..`turn_11`) to
  the speed-colored GPS lap (curvature/heading-rate peaks refine each apex). From the lap runs:
