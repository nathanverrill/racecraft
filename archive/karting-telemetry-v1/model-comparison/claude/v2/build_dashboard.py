...
def extract_track(t, E, N, speed, n_pts=260):
    """
    ...
    # rough revolution estimate (on all moving samples) to size the density gate
    def winding_split(mask):
        idx = np.where(mask)[0]
        if len(idx) < 40:
            return idx, [], np.array([])
        cx, cy = float(np.median(E[idx])), float(np.median(N[idx]))
        ang = np.unwrap(np.arctan2(N[idx] - cy, E[idx] - cx))
        base = ang - ang[0]
        if base[-1] < 0:                      # clockwise -> make it accumulate +
            base = -base
        bnds = [0]; target = 2 * np.pi
        for ii in range(1, len(idx)):
            if base[ii] >= target:
                bnds.append(ii); target += 2 * np.pi
        bnds.append(len(idx))
        loops = [idx[bnds[k]:bnds[k + 1]] for k in range(len(bnds) - 1)]
        return idx, loops, base

    _, rough_loops, _ = winding_split(moving)
    n_rev = max(1, len(rough_loops))
    ...
    idx, loops, base = winding_split(on_dense)

    # resample each full lap to a common arc-length parameterisation
...
#  Main processing
# --------------------------------------------------------------------------- #
def process(folder, out_hz, units="mph"):
    ...
    # ---- track silhouette (winding-split laps) + clean lap times ----
    track = extract_track(t_grid, Ef, Nf, speed)
    laps = track["laps"]
...
def main():
    ...
    payload = process(folder, args.hz, units=args.units)
