...
- `ingestion/output/gateway-kartplex/_venue/venue_landmarks.geojson` and `satellite_ref.json`.
...
- Commit a checkpoint after EACH pipeline step that RUNS and SELF-VALIDATES. Clear messages, e.g.
  "checkpoint(ingest): unzipped, 4 images classified, metadata read - OK".
...
1. ingest: scan inbox flat. Inputs = exactly one .zip + images (.heic/.jpg/.png); IGNORE stray
   files (.DS_Store, .sqlite, etc) gracefully. Unzip -> raw_sessions/<name>/. Read metadata.csv
   (device, schema v, recording epoch+tz, sensor list, per-sensor sampleRateMs). Vision-CLASSIFY
   the images: results-table = timing sheet (expect 2 here), hand-drawn = track map, top-down
   ortho = satellite. CONFIRM the zip actually contains Headphone.csv + Microphone.mp4 + Location/
   Accelerometer/Gyroscope/Gravity/Microphone.csv. VALIDATE: print file inventory + rates.
2. timesheet: vision-read the 2 sheets -> per sheet: datetime (AUTHORITATIVE, from header next to
   "Laptimes (10 min A&D)"), all printed fields (kart#, per-lap times, best, #laps, ProSkill,
   position, visit#/race# [capture but mark UNRELIABLE]) + handwritten notes verbatim ->
...
6. laps: seed gate from venue S/F coords -> snap to GPS track -> gate-crossing detection +
   AUTO-ALIGN to the timesheet (slide window, drop out-lap). Mark flyers; isolate out/in/pit using
   the pit-exit/entrance landmark corridors. VALIDATE: detected lap times match sheet (RMSE < ~0.3s,
   like v1's 0.2s).
